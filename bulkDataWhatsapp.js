...
function checkIfBulkDataIsNotProcessed() {
  return new Promise((resolve, reject) => {
    connection.query('SELECT * FROM bulkdata WHERE processed = 0 AND whatsappMode = true LIMIT 1', function (error, file) {
      if (!error) {
        if (file.length > 0) {
          resolve(file)
        } else {
...
function insertRegistries (data) {
  return new Promise((resolve, reject) => {
	...
	let { registries, idBulk } = data
        ...
        connection.query('INSERT INTO responsewapps (name, response, message) VALUES ?', [registries], function (error) {
          if (!error) {
